...
#[command(about = "🎙️ Download and synchronize podcasts from RSS feeds")]
#[command(version)]
struct Args {
    ...
    fn report(&self, event: ProgressEvent) {
        match event {
            ProgressEvent::FetchingFeed { url } => {
                self.main_bar
                    .set_message(format!("🔍 Fetching feed: {}", url.cyan()));
            }
            ...
            ProgressEvent::DownloadCompleted {
                ...
                let bar = self.get_or_create_bar(download_id);
                bar.set_position(bytes_downloaded);
                bar.set_message(format!("✅ {}", truncate_title(&episode_title, 40).green()));
...
async fn main() -> Result<()> {
    ...
    if !args.quiet && !result.failed_episodes.is_empty() {
        ...
        for (title, error) in &result.failed_episodes {
            println!("  {} {} - {}", "✗".red(), title.yellow(), error.dimmed());
        }
    }
